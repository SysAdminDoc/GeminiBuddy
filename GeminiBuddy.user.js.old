...
(function() {
    ...
    // --- EXECUTION GUARD ---
    // Prevents the script from running multiple times on the same page, which can happen with SPA navigation.
    ...
    const GM_SETTINGS_KEY = 'gemini_panel_settings_v24'; // Version bump for new settings
    ...
    function createCategory(categoryName, prompts, isCollapsible, isMini = false) {
        ...
        // Enable drag-and-drop for tag groups on the main panel
    ...
    function buildImportExportModal() {
        ...
        modalContent.append(modalHeader, modalBody);
        modal.appendChild(modalContent);
        modal.addEventListener('click', e => { if (e.target === modal) { modal.style.display = 'none'; lastFetchedUrl = null; }});
        return modal;
    }
    ...
    function initializeCopyActions() {
        copyResponseButton.addEventListener('click', async () => {
            ...
            if (allResponses.length > 0) {
                ...
                // Find the container with the rendered markdown text
    ...
    function initializePageObserver() {
        // Observer to detect when the main chat interface is available
        const pageObserver = new MutationObserver((mutationsList, observer) => {
            ...
            if (chatContainer) {
                ...
                initializeGenerationObserver(); // Start observing the send button
                updateNavigator();
            }
        });

        // Start observing the body for the chat container to be added
    ...
    function initializeGenerationObserver() {
        const setupObserver = () => {
            ...
            if (sendButton) {
                if(generationObserver) generationObserver.disconnect(); // Disconnect previous if any
        ...
        // The send button might also be dynamically loaded, so we watch for it
    ...
    function sendPromptToGemini(promptData, autoSend = false) {
        ...
        if (editor) {
            ...
            if (autoSend) {
                setTimeout(() => {
                    // Use a more generic selector for the send button
