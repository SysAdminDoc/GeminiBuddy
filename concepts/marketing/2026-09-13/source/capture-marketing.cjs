...
const repoRoot = 'C:\\repos\\GeminiBuddy';
const extensionSource = path.join(repoRoot, 'chrome_extension', 'dist', 'geminibuddy-mv3');
const captureDir = path.resolve(__dirname, '..', 'screenshots');
...
async function captureScreenshot(session, filename) {
  ...
  fs.writeFileSync(path.join(captureDir, filename), Buffer.from(data, 'base64'));
}
  ...
  shim = shim.replace(
    ...
    if (key === 'gemini_custom_prompts_v6' && typeof defaultValue === 'undefined') {
      return Promise.resolve(JSON.stringify({
        Research: [
          { id: 'research-brief', name: 'Research brief', text: 'Build a focused research brief with sources, risks, and open questions.' },
          { id: 'compare-options', name: 'Compare options', text: 'Compare these options by cost, effort, tradeoffs, and fit.' }
        ],
        Writing: [
          { id: 'tighten-draft', name: 'Tighten this draft', text: 'Rewrite this draft so it is clear, concise, and direct.' },
          { id: 'project-update', name: 'Project update', text: 'Turn these notes into a short project update with decisions and next actions.' }
        ]
      }));
    }`
  );
...
async function main() {
  ...
  const fixtureServer = http.createServer((request, response) => {
    ...
    response.end(`<!doctype html><html><head><style>
      *{box-sizing:border-box}body{margin:0;background:#101116;color:#e9eaf0;font-family:Arial,sans-serif}
      header{height:64px;border-bottom:1px solid #2b2d36;display:flex;align-items:center;padding:0 28px;font-weight:700}
      main{width:min(820px,calc(100% - 420px));margin:54px auto 0;padding:0 24px}
      .greeting{font-size:34px;font-weight:700;margin-bottom:12px}.sub{color:#a9acb8;font-size:17px;margin-bottom:30px}
      .chat-history{display:grid;gap:14px}.card{background:#1b1d25;border:1px solid #30333d;border-radius:18px;padding:18px 20px;color:#cfd2dc}
      rich-textarea{display:block;height:126px;margin-top:28px;border:1px solid #3a3d49;border-radius:22px;background:#1a1c23}
    </style></head><body><header>Gemini</header><main><div class="greeting">What can I help you build?</div><div class="sub">Start with a saved prompt, then keep the useful ones close.</div><div class="chat-history"><div class="card">Plan a focused research brief for a new product idea.</div><div class="card">Turn scattered notes into a clear project update.</div></div><rich-textarea></rich-textarea></main></body></html>`);
  });
  await new Promise((resolve, reject) => {
  ...
  try {
    ...
    await setViewport(pageSession, 1440, 900);
    await evaluate(pageSession, "document.querySelector('#gemini-prompt-panel-main').classList.add('visible')");
    await wait(350);
    await captureScreenshot(pageSession, 'injected-panel.png');
    ...
    await setViewport(pageSession, 1440, 900);
    await wait(200);
    await captureScreenshot(pageSession, 'options.png');
    ...
    await setViewport(pageSession, 420, 900);
    await wait(200);
    await captureScreenshot(pageSession, 'side-panel.png');

    await wait(500);
    if (consoleErrors.length) throw new Error(`Browser console errors:\n${consoleErrors.join('\n')}`);
    console.log(`Marketing capture passed: ${captureDir} (${extensionId}).`);
  } finally {
    pageSession?.close();
